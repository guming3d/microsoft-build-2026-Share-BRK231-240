...
```mermaid
flowchart LR
    subgraph Build["Build in GitHub"]
        B1["Code"]
        B2["Prompts"]
        B3["Skills + tools"]
        B4["Eval assets"]
    end
    subgraph Run["Run in Foundry"]
        R1["Agent runtime<br/>plan / act / observe<br/>host & scale"]
        R2["Evaluate & Optimize<br/>routing + RL<br/>agents & eval signals"]
    end
    subgraph Distribute["Distribute to users"]
        D1["M365 Copilot"]
        D2["Teams"]
        D3["Apps + APIs"]
    end
    ...
    Levers["Faster, Better, Cheaper:<br/>Prompting · Context Management · Tools Handling · Model Fine-tuning (SFT, RFT, RL)"]
    Backed["Backed by: AI Models (11K) · Context with IQ · Security + Control"]
